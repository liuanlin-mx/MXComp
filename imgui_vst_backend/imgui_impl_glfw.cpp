...
static std::mutex g_ImGui_ImplMutex;
static std::list<ImGui_ImplEvent> g_ImGui_ImplEvents;
...
void ImGui_ImplGlfw_MouseButtonCallback(GLFWwindow* window, int button, int action, int mods)
{
    ...
    if (window != bd->Window)
    {
        std::lock_guard<std::mutex> l(g_ImGui_ImplMutex);
        ImGui_ImplEvent event;
        event.window = window;
        event.cb = std::bind(ImGui_ImplGlfw_MouseButtonCallback, window, button, action, mods);
        g_ImGui_ImplEvents.push_back(event);
        return;
    }
...
void ImGui_ImplGlfw_ScrollCallback(GLFWwindow* window, double xoffset, double yoffset)
{
    ...
    if (window != bd->Window)
    {
        ...
        ImGui_ImplEvent event;
        event.window = window;
        event.cb = std::bind(ImGui_ImplGlfw_ScrollCallback, window, xoffset, yoffset);
        g_ImGui_ImplEvents.push_back(event);
        return;
    }
...
void ImGui_ImplGlfw_KeyCallback(GLFWwindow* window, int keycode, int scancode, int action, int mods)
{
    ...
    if (window != bd->Window)
    {
        ...
        ImGui_ImplEvent event;
        event.window = window;
        event.cb = std::bind(ImGui_ImplGlfw_KeyCallback, window, keycode, scancode, action, mods);
        g_ImGui_ImplEvents.push_back(event);
        return;
    }
...
void ImGui_ImplGlfw_WindowFocusCallback(GLFWwindow* window, int focused)
{
    ...
    if (window != bd->Window)
    {
        ...
        ImGui_ImplEvent event;
        event.window = window;
        event.cb = std::bind(ImGui_ImplGlfw_WindowFocusCallback, window, focused);
        g_ImGui_ImplEvents.push_back(event);
        return;
    }
...
void ImGui_ImplGlfw_CursorPosCallback(GLFWwindow* window, double x, double y)
{
    ...
    if (window != bd->Window)
    {
        ...
        ImGui_ImplEvent event;
        event.window = window;
        event.cb = std::bind(ImGui_ImplGlfw_CursorPosCallback, window, x, y);
        g_ImGui_ImplEvents.push_back(event);
        return;
    }
...
void ImGui_ImplGlfw_CursorEnterCallback(GLFWwindow* window, int entered)
{
    ...
    if (window != bd->Window)
    {
        ...
        ImGui_ImplEvent event;
        event.window = window;
        event.cb = std::bind(ImGui_ImplGlfw_CursorEnterCallback, window, entered);
        g_ImGui_ImplEvents.push_back(event);
        return;
    }
    if (glfwGetInputMode(window, GLFW_CURSOR) == GLFW_CURSOR_DISABLED)
...
void ImGui_ImplGlfw_CharCallback(GLFWwindow* window, unsigned int c)
{
    ...
    if (window != bd->Window)
    {
        ...
        ImGui_ImplEvent event;
        event.window = window;
        event.cb = std::bind(ImGui_ImplGlfw_CharCallback, window, c);
        g_ImGui_ImplEvents.push_back(event);
        return;
    }
...
void ImGui_ImplGlfw_PollEvents()
{
    ImGui_ImplGlfw_Data* bd = ImGui_ImplGlfw_GetBackendData();
        
    std::lock_guard<std::mutex> l(g_ImGui_ImplMutex);
    for (auto it = g_ImGui_ImplEvents.begin(); it != g_ImGui_ImplEvents.end();)
    {
        if (glfwGetTime() - it->time > 1.0)
        {
            it = g_ImGui_ImplEvents.erase(it);
            continue;
        }
        
        if (it->window == bd->Window)
        {
            it->cb();
            it = g_ImGui_ImplEvents.erase(it);
        }
